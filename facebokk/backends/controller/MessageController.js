...
export const getMessages = async (req, res) => {
  try {
    ...
    const userId = req.user._id; // auth middleware se aana chahiye
    ...
    // 3️⃣ Mark unseen as read
    await Message.updateMany(
      { conversationId, receiverId: userId, read: false },
      { $set: { read: true } }
    );

    // 4️⃣ Notify via socket (future blue tick)
    if (global.io) {
      conversation.members.forEach((id) => {
        global.io.to(id.toString()).emit("messagesRead", {
          conversationId,
          readerId: userId,
        });
      });
